...
import type { Route } from "@/store/modules/types";
import {
  menuTypeOptions,
  showLinkOptions,
  keepAliveOptions,
...
<template>
  ...
  >
    <el-row :gutter="30">
      ...
      <re-col v-if="newFormInline.menuType !== 3" :value="12" :xs="24" :sm="24">
        <!-- 按钮级别权限设置 -->
        <el-form-item label="Auth选项" prop="auths_options">
          <el-select
            v-model="newFormInline.meta.auths_options"
            multiple
            allow-create
          ...
          >
            <el-option
              v-for="item in newFormInline.authsOptions"
              :key="item._id"
              :label="item.name"
              :value="item._id"
            />
